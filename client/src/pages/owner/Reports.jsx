...
const Reports = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const salesmenData = await fetch('/api/users?role=salesman').then(res => res.json());
        ...
        setError('Failed to load data for report options. Please try again later.');
      }
    };
  ...
  const generateReport = async () => {
    try {
      ...
      switch (reportType) {
        case 'financial':
          response = await reportService.generateFinancialReport(startDate, endDate, format);
          break;
        case 'inventory':
          response = await reportService.generateInventoryReport(startDate, endDate, format);
          break;
        case 'salesman':
          if (!entityId) {
          ...
          response = await reportService.generateSalesmanReport(entityId, startDate, endDate, format);
          break;
        case 'product':
          if (!entityId) {
          ...
          response = await reportService.generateProductReport(entityId, startDate, endDate, format);
          break;
        case 'sales-summary':
          response = await reportService.getSalesSummary(startDate, endDate);
          break;
        case 'product-performance':
          response = await reportService.getProductPerformance(startDate, endDate, entityId || null);
          break;
        case 'salesman-performance':
          response = await reportService.getSalesmanPerformance(startDate, endDate, entityId || null);
          break;
        case 'shop-performance':
          response = await reportService.getShopPerformance(startDate, endDate, entityId || null);
          break;
      ...
      if (format === 'json') {
        ...
        setSuccess('Report generated successfully. Your download should begin shortly.');
      }
    } catch (error) {
  ...
  const needsEntitySelection = ['salesman', 'product', 'salesman-performance', 'product-performance', 'shop-performance'].includes(reportType);
                  ...
                  <div>
                    ...
                    <Select value={reportType} onValueChange={setReportType}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="salesman">Salesman Report</SelectItem>
                  ...
                  <div>
                    ...
                    <Select value={format} onValueChange={setFormat}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="json">JSON (View in Console)</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                </div>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="grid gap-4">
                  ...
                  <div>
                    ...
                    <Select value={reportType} onValueChange={setReportType}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="salesman-performance">Salesman Performance</SelectItem>
                        <SelectItem value="shop-performance">Shop Performance</SelectItem>
                        <SelectItem value="inventory-status">Inventory Status</SelectItem>
                        <SelectItem value="returns-analysis">Returns Analysis</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                ...
                {needsEntitySelection && (
                  <div>
                    <Label htmlFor="entity-id">{getEntityLabel()} (Optional)</Label>
                    <Select value={entityId} onValueChange={setEntityId}>
                      <SelectTrigger id="entity-id">
                        <SelectValue placeholder={`Select ${getEntityLabel().toLowerCase()} or leave blank for all`} />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="">All {getEntityLabel()}s</SelectItem>
                        {getEntityOptions()}
                      </SelectContent>
                    </Select>
                  </div>
                ...
                {['sales-summary', 'product-performance', 'salesman-performance', 'shop-performance', 'returns-analysis'].includes(reportType) && (
                  <div className="grid md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label>Start Date</Label>
                      <Popover>
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-gray-500 mb-4">
                Build custom reports by selecting specific metrics, dimensions, and filters. This feature allows you to create tailored reports to meet your specific business analysis needs.
              </p>
              
              <div className="bg-yellow-50 p-4 border border-yellow-200 rounded-md mb-6">
                <p className="text-yellow-800">
                  The custom report builder is coming soon. Please use the standard reports and performance reports tabs for now.
                </p>
              </div>
            ...
            <table className="w-full border-collapse">
              ...
              <tbody>
                ...
                <tr className="bg-white">
                  ...
                  <td className="border p-2">{format(new Date(new Date().setDate(new Date().getDate() - 2)), 'PPP')}</td>
                  <td className="border p-2 text-center">
                    <Button variant="outline" size="sm">Download</Button>
                  </td>
                </tr>
