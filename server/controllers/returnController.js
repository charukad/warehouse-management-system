...
const getReturnById = async (req, res, next) => {
  try {
    ...
    if (
      ...
      (req.user.role === 'shop' && returnRecord.shop && returnRecord.shop.toString() !== req.user.id) &&
      (req.user.role === 'salesman' && 
        returnRecord.salesman.toString() !== req.user.id && 
        returnRecord.processed_by.toString() !== req.user.id)
    ) {
      return ApiResponse.forbidden(res, 'You are not authorized to view this return');
    }
    
    // Get return items
    const items = await ReturnItem.find({ return: id })
      .populate('product', 'product_name product_code retail_price wholesale_price image_url');
    ...
    if (returnRecord.shop) {
      shop = await Shop.findById(returnRecord.shop)
        .select('shop_name address contact_person phone_number');
    }
    
    if (returnRecord.salesman) {
      salesman = await User.findById(returnRecord.salesman)
        .select('fullName username');
    }
    
    if (returnRecord.order) {
      order = await Order.findById(returnRecord.order)
        .select('reference_number order_date total_amount');
    }
    
    if (returnRecord.processed_by) {
      processor = await User.findById(returnRecord.processed_by)
...
const createShopReturn = async (req, res, next) => {
  ...
  try {
    ...
    if (req.user.role !== 'salesman') {
      return ApiResponse.forbidden(res, 'Only salesmen can process shop returns');
    }
    ...
    if (shop.created_by_salesman.toString() !== req.user.id) {
      return ApiResponse.forbidden(res, 'You are not authorized to process returns for this shop');
    }
    ...
    if (order_id) {
      ...
      if (order.shop.toString() !== shop_id || order.salesman.toString() !== req.user.id) {
        return ApiResponse.forbidden(res, 'Order does not belong to this shop or salesman');
      }
    }
    ...
    for (const item of items) {
      if (!item.product_id || !item.quantity || !item.condition) {
        return ApiResponse.error(res, 'Product ID, quantity, and condition are required for each item', 400);
      }
      
      // Check if product exists
      const product = await Product.findById(item.product_id);
      if (!product) {
        return ApiResponse.error(res, `Product with ID ${item.product_id} not found`, 400);
      }
    }
    
    // Generate reference number
    const reference_number = `RET-${Date.now().toString().slice(-6)}-${Math.floor(Math.random() * 1000)}`;
...
const createSalesmanReturn = async (req, res, next) => {
  ...
  try {
    ...
    if (req.user.role !== 'warehouse_manager') {
      return ApiResponse.forbidden(res, 'Only warehouse managers can process salesman returns');
    }
    ...
    for (const item of items) {
      if (!item.product_id || !item.quantity) {
        return ApiResponse.error(res, 'Product ID and quantity are required for each item', 400);
      }
      ...
      if (!salesmanInventory || salesmanInventory.remaining_quantity < item.quantity) {
        return ApiResponse.error(
          res, 
          `Salesman does not have enough ${product.product_name}. Available: ${salesmanInventory?.remaining_quantity || 0}`, 
          400
        );
      }
    }
    ...
    // Generate reference number
    const reference_number = `EOD-${Date.now().toString().slice(-6)}-${Math.floor(Math.random() * 1000)}`;
    ...
    for (const item of items) {
      ...
      if (!inventory) {
        ...
        inventory.allocated_stock = Math.max(0, inventory.allocated_stock - item.quantity);
...
const getReturnsByShop = async (req, res, next) => {
  try {
    ...
    if (
      req.user.role !== 'owner' && 
      req.user.role !== 'warehouse_manager' && 
      (req.user.role === 'shop' && shop.user && shop.user.toString() !== req.user.id) &&
      (req.user.role === 'salesman' && shop.created_by_salesman.toString() !== req.user.id)
    ) {
      return ApiResponse.forbidden(res, 'You are not authorized to view this shop\'s returns');
    }
    ...
    const returnsWithItems = await Promise.all(returns.map(async (ret) => {
      const items = await ReturnItem.find({ return: ret._id })
    ...
    const totalAmount = returnsWithItems.reduce((sum, ret) => sum + (ret.total_amount || 0), 0);
    const totalItems = returnsWithItems.reduce((sum, ret) => sum + ret.item_quantity, 0);
...
const getReturnsBySalesman = async (req, res, next) => {
  try {
    ...
    if (
      ...
      return ApiResponse.forbidden(res, 'You are not authorized to view this salesman\'s returns');
    }
    ...
    const returnsWithDetails = await Promise.all(returns.map(async (ret) => {
      const items = await ReturnItem.find({ return: ret._id })
        .populate('product', 'product_name product_code');
      
      let shopDetails = null;
      if (ret.shop) {
        shopDetails = await Shop.findById(ret.shop)
    ...
    const shopReturns = returnsWithDetails.filter(ret => ret.return_type === 'shop');
    const salesmanReturns = returnsWithDetails.filter(ret => ret.return_type === 'salesman');
    
    const stats = {
      total_returns: returns.length,
      shop_returns: {
        count: shopReturns.length,
        total_amount: shopReturns.reduce((sum, ret) => sum + (ret.total_amount || 0), 0),
        total_items: shopReturns.reduce((sum, ret) => sum + ret.item_quantity, 0)
      },
      salesman_returns: {
        count: salesmanReturns.length,
        total_amount: salesmanReturns.reduce((sum, ret) => sum + (ret.total_amount || 0), 0),
        total_items: salesmanReturns.reduce((sum, ret) => sum + ret.item_quantity, 0)
      }
    };
...
const getReturnsAnalysis = async (req, res, next) => {
    try {
      ...
      if (req.user.role !== 'owner') {
        return ApiResponse.forbidden(res, 'Only owners can access returns analysis');
      }
      ...
      const returnsByType = await Return.aggregate([
        ...
        },
        {
          $group: {
            _id: {
              date: { $dateToString: { format: "%Y-%m-%d", date: "$return_date" } },
              type: "$return_type"
            },
            count: { $sum: 1 },
            ...
            value_return_rate: {
              $cond: {
                ...
                then: 0,
                  ...
                  $multiply: [
                    { $divide: [{ $sum: '$returns.total_amount' }, { $sum: '$orders.total_amount' }] },
                    100
                  ]
                }
              }
      ...
      const analysis = {
        ...
        financial_impact: {
          ...
          return_ratio: totalSales.length > 0 && totalSales[0].total_sales > 0 
            ? (totalReturns.length > 0 ? (totalReturns[0].total_returns / totalSales[0].total_sales) * 100 : 0) 
            : 0,
          order_count: totalSales.length > 0 ? totalSales[0].order_count : 0,
          return_count: totalReturns.length > 0 ? totalReturns[0].return_count : 0
        }
      };
